...
def _divide_ranges(
    ...
    the new size; the ``SpyreTensorLayout`` is scaled by dividing the device
    dimension whose stride_map entry matches each tiled host dimension, leaving
    stride_map unchanged so restickified layouts are preserved.
    """
    data = op.data
